...
class AsyncRuTrackerClient(BaseRuTrackerClient):
    ...
    async def _send_post_request(self, url: str, data: list) -> str:
        """
        Отправляет POST-запрос с поддержкой retry и возвращает текст ответа.
        ...
        try:
            ...
            async with self.session.post(
                url, data=form_data, ssl=self._ssl_context, proxy=self.proxy, timeout=aiohttp.ClientTimeout(total=30)
            ) as response:
    ...
    async def search_with_form(
        ...
        if page == 1:
            ...
            try:
                content = await self._send_post_request(url, data)
            except RuTrackerRequestError as ex:
            ...
            try:
                results = self._parse_search_results(content, return_search_dict)
                search_id = self._extract_search_id(content)
                if search_id:
                ...
                if not hasattr(ex, 'html_snippet') or ex.html_snippet is None:
                    html_snippet = content[:200] if content else None
                    if html_snippet:
    ...
    async def search_all_pages_with_form(
        self,
        title: str,
        return_search_dict: bool = False,
        max_pages: Optional[int] = None,
        ...
        while page <= max_pages:
            try:
                results = await self.search_with_form(
                    title=title,
                    page=page,
