"""
Пример использования вспомогательных методов SearchFormData для удобного поиска значений (асинхронный клиент).
...
async def main():
    async with AsyncRuTrackerClient(LOGIN, PASSWORD, PROXY) as client:
        ...
        print("=" * 60)
        print("ИСПОЛЬЗОВАНИЕ ВСПОМОГАТЕЛЬНЫХ МЕТОДОВ SearchFormData (ASYNC)")
        print("=" * 60)
